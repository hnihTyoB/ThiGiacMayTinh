...
## 5. Chuẩn bị Dataset
Nếu bạn muốn tự chuẩn bị dữ liệu hoặc chạy ở máy cá nhân, sử dụng script tải tự động hoặc tạo ảnh blur:
```bash
# Tải bộ dữ liệu mẫu
python scripts/data_preparation/download_deblur_datasets.py --all

# Hoặc tự tạo ảnh blur từ tập ảnh sắc nét có sẵn
python scripts/data_preparation/create_blur_dataset.py --input <đường_dẫn_sharp> --output <đường_dẫn_blur> --task idcard
```
